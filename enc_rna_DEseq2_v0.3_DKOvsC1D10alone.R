...
files <- c("results/C1B6-A/rsem_quant/rep1/rep1PE_stranded_anno_rsem.isoforms.ENSG.results",
...
names(files) <- c("C1B6.A","C1B6.B","C1B6.C","C1D10.A","C1D10.B","C1D10.C")
...
sampleTable <- data.frame(condition = factor(c("DKO","DKO","DKO","SKO","SKO","SKO")))

rownames(sampleTable) <- colnames(txi$counts)

dds <- DESeqDataSetFromTximport(txi, sampleTable, ~condition)
dds$condition <- relevel(dds$condition,ref="SKO")
...
vsd$gene <- gsub("\\..*","",rownames(vsd))
vsd <- as_tibble(vsd)

rld <- as.data.frame(assay(rlog(dds, blind=FALSE)))
rld$gene <- gsub("\\..*","",rownames(rld))
rld <- as_tibble(rld)

counts <- as.data.frame(counts(dds))
counts$gene <- gsub("\\..*","",rownames(counts))
counts <- as_tibble(counts)

write_tsv(vsd,"DEseq2_VSD_DKOvsC1D10_022423.tsv", col_names = TRUE)
write_tsv(rld,"DEseq2_RLD_DKOvsC1D10_022423.tsv", col_names = TRUE)
write_tsv(counts,"DEseq2_sfCounts_DKOvsC1D10_022423.tsv", col_names = TRUE)


DKOvSKO <- results(dseq_obj, contrast=c("condition", "SKO", "DKO")) #,

res <- lfcShrink(dseq_obj,
                 contrast=c("condition", "DKO", "SKO"), res=DKOvSKO, type = 'normal')
...
write_tsv(t,"DEseq2_lfc_DKOvsC1D10alone_022023.tsv", col_names = TRUE)
...
tz_ty_tab <- read_tsv("../analysis/abc/just_peaks/gene_list_forGSEA_022723.tsv", col_names = TRUE)

filt <- c("HERC3","TBCE")

res2 <- t.protein_coding %>% 
  dplyr::select(SYMBOL, stat,gene) %>% 
  na.omit() %>%
  dplyr::filter(!SYMBOL %in% filt) %>%
  inner_join(tz_ty_tab, by = c('gene')) %>%
  distinct()
#%>% 
  #distinct() #%>% 
  #group_by(SYMBOL) %>% 
