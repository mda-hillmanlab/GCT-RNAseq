...
names(files) <- c("C1B6.A","C1B6.B","C1B6.C","C1D10.A","C1D10.B","C1D10.C","B2D8.A","B2D8.B","B2D8.C")
...
sampleTable <- data.frame(condition = factor(c("DKO","DKO","DKO","SKO","SKO","SKO","SKO","SKO","SKO")))
...
write_tsv(t,"DEseq2_lfc_DKOvsSKO_021923.tsv", col_names = TRUE)
...
plotEnrichment(pathways.hallmark[["HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"]],
               ranks) + labs(title="Epithelial Mesenchymal Transition")
...
assayed.genes <- gsub("\\..*","",rownames(DKOvSKO))
de.genes <- gsub("\\..*","",rownames(DKOvSKO)[ which(DKOvSKO$padj < .1) ])

pwf=nullp(assayed.genes,"hg38","ensGene")
